...
duration = 50
lambds = [0., 0.999, 0.99995]

for lambd in lambds[:1]: ## stop at first one for testing purposes
    for markov_model_file in files[:5]: ## stop at 5 files for testing purposes
        ...
        name = 'hybrid_QSD_' + str(seed) + "_" + str(duration) + "_" + str(lambd) + "_" + hash_name + "_" + markov_seed  + "_" + n_clusters  + "_" + n_iter
        ...
        if OVERWRITE or not file_exists:
            ...
            filey.writelines("python /scratch/users/tabakg/qsd_dev/hybrid_qsd.py --seed %s --lambd %s --duration %s --markov_file '%s' --diffusion_file '%s' --output_file_path '%s' --save2pkl" % (seed, lambd, duration, markov_model_path, diffusion_maps_path, output_file_path))
            filey.close()
