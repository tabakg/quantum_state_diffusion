...
This code simulates either one open quantum system with heterodyne detectors,
or two systems with partially coherent transmission (will be discussed in
upcoming notes). The input used for this code is a json file (spec_file below)
which specifies several parameters as well as the important operators required.
This file can be generated by gen_num_model.py in the parent directory
(https://github.com/tabakg/quantum_state_diffusion/blob/master/generate_num_model.py).

Once this code completes running it should generate two json files: psis_out
and expects_out. The psis_out is a 3-dim tensor of dimensions
(num of trajectories, num of timesteps, dimensions). The expects_out is a tensor
of dimensions (num of trajectories, num of timesteps, num of observables)
representing all desired expectation values for the observables.
...
void run_trajectory(system_type * system_ptr, int seed, int steps_for_noise,
  ...
  std::cout << "num_downsampled_steps * system.downsample: " << num_downsampled_steps * system.downsample << std::endl;
